## Disable Non-Essential Services on Kiosk Tier 2 ##########
...
Write-Output Yes | REG ADD "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows Embedded\EmbeddedLogon" /v BrandingNeutral /t REG_DWORD /d 8 /f
Write-Output Yes | REG ADD "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" /v dontdisplaylastusername /t REG_DWORD /d 1 /f
...
# Fix Shell
Function FixShell {
	Write-Output "Installing Shell Experience Service..."
	Get-appxpackage -AllUsers "*shellexperience*" -packagetype bundle | ForEach {Add-AppxPackage -DisableDevelopmentMode -Register "$($_.InstallLocation)\AppXManifest.xml"}
}
##########
